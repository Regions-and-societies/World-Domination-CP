...
| `TryGetLevel` | `bool TryGetLevel(WorldObject obj, out int level, out int maxLevel)` | Reports a WD faction settlement's grade — see "Settlement grade → level" below. **Suppressed this release** (see "Sizing suppression"). |

### Sizing suppression (this release)

For this release the adapter ships **classification only**: `WorldDominationAdapter.SizingSuppressed` gates off the sizing inputs — the settlement grade → level mapping, the WD-managed settlement classification takeover that exists only to feed it, and the outpost pawn-count population. R&S therefore never resizes a World Domination holding; WD's own settlement and outpost mechanics stay fully authoritative, exactly as in 0.1.0's settlement handling. The implementations below remain compiled and documented as designed; flipping the flag to `false` re-enables them, targeted at the next release together with the in-game validation (#3) and outpost grading (#4).
...
| WD-managed `Settlement` (see below) | `Settlement` | A vanilla `Settlement` WD grades. We take ownership only so core will read our `TryGetLevel` for it; the kind is identical to what the vanilla adapter would return. **Skipped while sizing is suppressed** — with no level to feed, WD bases stay with the vanilla adapter. |
...
Implemented only for `WorldObject_WD_Outpost`: returns the type's own `PawnCount` member, read typed. Note that `WorldObject_WD_Outpost` does **not** derive from `Outposts.Outpost` (Vanilla Outposts Expanded) — the shared member name is a coincidence, verified by compile. **Suppressed this release** (see "Sizing suppression"): the adapter declines, and core falls back to its own defaults.

### Settlement grade → level

> **Suppressed this release** (see "Sizing suppression"). The design below is implemented and compiled, gated off behind `SizingSuppressed`, and targeted to go live next release after in-game validation (#3).
